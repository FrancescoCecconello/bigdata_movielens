...
# Calcolo il rating medio per ogni film. Per farlo raggruppo i rating per "movieId"
# e poi applico la funzione avg, rinominando la colonna generata "AverageRating"
average_ratings_per_movie = ratings.groupBy("movieId").agg(avg("rating").alias("AverageRating"))

# Mostro la tabella con il rating medio per film
average_ratings_per_movie.show()

# Calcolo la media delle valutazioni per ogni film e il numero totale di rating per ogni gruppo.
# Raggruppo i rating per "movieId", poi genero due nuove colonne. Nella prima colonna "Average"
# inserisco il rating medio per film, nella seconda colonna "NumRatings" inserisco il numero
# di rating per film
average_ratings = ratings.groupBy("movieId").agg(mean(col("Rating")).alias("Average"),count(col("Rating")).alias("NumRatings")
)
...
plt.xlabel("Rating medio per film")
